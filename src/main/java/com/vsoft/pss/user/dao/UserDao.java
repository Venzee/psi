...
public class UserDao extends BaseDao {
	...
	public void addUser(Map<String, Object> data) {
		try {
			this.insertToTable("pss_user", data);
			LOG.info("用户注册成功");
	...
	public int addUserReturnId(Map<String, Object> data) {
		int id = 0;
		try {
			id = this.insertAndReturnPrimaryId("pss_user", data);
			LOG.info("用户注册成功");
		} catch (SQLException e) {
			LOG.info("用户注册时出错", e);
			e.printStackTrace();
	...
	public Map<String, Object> queryUserForLogin(List<Object> params) {
		String sql = "select u.id,u.username,u.password,u.groupId,u.employeeId,u.companyId from pss_user u where u.username = ? and u.password = ?";
		try {
